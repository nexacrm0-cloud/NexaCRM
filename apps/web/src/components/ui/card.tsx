...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'border-ink/14 bg-receipt text-card-foreground shadow-recibo rounded-[2px] border',
        className,
      )}
      {...props}
    />
...
const CardEyebrow = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn('eyebrow border-ink/14 border-b pb-3', className)} {...props} />
  ),
);
